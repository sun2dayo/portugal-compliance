        ...
        for doctype in compliance_doctypes:
            if frappe.db.table_exists(f"tab{doctype}"):
                ...
                if not has_permission:
                    frappe.logger().debug(f"User {frappe.session.user} has no permission for {doctype}")
...
def log_authentication_attempt():
    ...
    try:
        ...
        frappe.cache.set(f"portugal_auth_log_{frappe.session.user}_{now()}", audit_data, expires_in_sec=86400)
...
def get_user_compliance_info():
    ...
    try:
        ...
        for doctype in compliance_doctypes:
            if frappe.db.table_exists(f"tab{doctype}"):
                user_info["compliance_permissions"][doctype] = frappe.has_permission(doctype, "read")
