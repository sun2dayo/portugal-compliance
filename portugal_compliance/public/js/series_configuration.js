...
/**
 * Series Configuration JS - Portugal Compliance VERSÃO NATIVA CORRIGIDA
 * Configuração centralizada de séries portuguesas para todos os documentos
 ...
 * ✅ Geração automática de séries comunicadas
 * ✅ Interface unificada de configuração de séries
 * ✅ INCLUÍDO: POS Invoice e todos os documentos aplicáveis
 */
...
const DOCUMENT_TYPES = {
    ...
    'Quotation': {
        ...
        required_fields: ['customer', 'items', 'valid_till'],
        default_sequence: 1,
        communication_required: true
    },
    'Delivery Note': {
    ...
    'Purchase Invoice': {
        ...
        required_fields: ['supplier', 'items', 'bill_no', 'bill_date'],
        default_sequence: 1,
        communication_required: true
    },
    'Purchase Order': {
    ...
    'Stock Entry': {
        ...
        required_fields: ['stock_entry_type', 'items'],
        default_sequence: 1,
        communication_required: true
    },
    ...
    'Payment Entry': {
        ...
        required_fields: ['party', 'paid_amount', 'mode_of_payment'],
        default_sequence: 1,
        communication_required: true
    }
};
...
function setup_filters(frm) {
    /**
     * Configurar filtros para campos
     */
    ...
    // ✅ FILTRO PARA TIPOS DE DOCUMENTO
    frm.set_query("document_type", function() {
        return {
            filters: {
                "name": ["in", Object.keys(DOCUMENT_TYPES)]
            }
        };
    });
}
...
function setup_custom_fields(frm) {
    ...
    if (frm.fields_dict.prefix) {
        frm.fields_dict.prefix.df.description = "Prefixo da série (formato: XXYYYY + EMPRESA, ex: FT2025NDX)";
    ...
    if (frm.fields_dict.validation_code) {
        frm.fields_dict.validation_code.df.description = "Código de validação da AT (obtido após comunicação)";
    }
}
...
function setup_interface(frm) {
    /**
     * Configurar interface do formulário
     */
...
function add_series_info_section(frm) {
    ...
    let doc_info = DOCUMENT_TYPES[frm.doc.document_type] || {};
    let status = get_series_status(frm);
...
function add_document_info_section(frm) {
    /**
     * Adicionar seção de informações do documento
     */
    ...
    let doc_html = `
        <div class="document-info-section" style="
            background: #fff3e0;
        ...
        ">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <strong>Campos Obrigatórios:</strong> ${doc_info.required_fields.length}<br>
                    ${doc_info.limit_without_nif ? `<strong>Limite s/ NIF:</strong> €${doc_info.limit_without_nif}` : ''}
                </div>
            </div>
        </div>
    `;
    ...
    // ✅ ADICIONAR HTML AO FORMULÁRIO
    if (!frm.document_info_added) {
        $(frm.fields_dict.document_type.wrapper).after(doc_html);
        frm.document_info_added = true;
    }
}
...
function add_custom_buttons(frm) {
    ...
    // ✅ BOTÃO PARA CONFIGURAR TODAS AS SÉRIES
    if (frappe.user.has_role('System Manager')) {
        frm.add_custom_button(__('Configurar Todas as Séries'), function() {
            setup_all_series_for_company(frm);
        }, __('Configuração'));
    }
}
...
function validate_series_configuration(frm) {
    ...
    } else if (!DOCUMENT_TYPES[frm.doc.document_type]) {
        errors.push(__('Tipo de documento não suportado'));
    }
...
function show_document_info(frm) {
    /**
    ...
    if (frm.document_info_added) {
        $('.document-info-section').remove();
        frm.document_info_added = false;
        add_document_info_section(frm);
    }
}
...
function communicate_series_to_at(frm) {
    ...
    frappe.confirm(
        ...
        function() {
            frappe.call({
                method: 'portugal_compliance.api.communicate_series_to_at',
                args: {
...
function test_series_generation(frm) {
    ...
    frappe.call({
        method: 'portugal_compliance.api.test_series_generation',
        args: {
            series_config: frm.doc.name
        },
        callback: function(r) {
...
function setup_all_series_for_company(frm) {
    /**
     * ✅ NOVA: Configurar todas as séries para empresa
     */

    if (!frm.doc.company) {
        frappe.msgprint(__('Selecione uma empresa primeiro'));
        return;
    }

    frappe.confirm(
        __('Configurar séries para TODOS os tipos de documento para a empresa {0}?', [frm.doc.company]),
        function() {
            frappe.call({
                method: 'portugal_compliance.api.setup_all_series_for_company',
                args: {
                    company: frm.doc.company
                },
                callback: function(r) {
                    if (r.message && r.message.success) {
                        frappe.show_alert({
                            message: __('Criadas {0} séries para {1}', [r.message.created_count, frm.doc.company]),
                            indicator: 'green'
                        });

                        // ✅ MOSTRAR LISTA DAS SÉRIES CRIADAS
                        show_created_series_dialog(r.message.created_series);
                    } else {
                        frappe.msgprint({
                            title: __('Erro'),
                            message: r.message ? r.message.error : __('Erro ao criar séries'),
                            indicator: 'red'
                        });
                    }
                }
...
function show_created_series_dialog(created_series) {
    /**
     * ✅ NOVA: Mostrar séries criadas
     */
    ...
    let dialog = new frappe.ui.Dialog({
        ...
        fields: [
            {
                fieldtype: 'HTML',
                fieldname: 'created_series_list'
            }
        ]
    });

    let html = `
        <div class="created-series-list">
            <h5>Séries Criadas com Sucesso</h5>
            <table class="table table-striped">
                <thead>
                    <tr><th>Documento</th><th>Prefixo</th><th>Naming Series</th><th>Status</th></tr>
                </thead>
                <tbody>
    `;

    created_series.forEach(function(series) {
        html += `
            <tr>
                <td>${DOCUMENT_TYPES[series.document_type]?.name || series.document_type}</td>
                <td>${series.prefix}</td>
                <td>${series.naming_series}</td>
                <td><span class="indicator green">Criada</span></td>
            </tr>
        `;
    });
    ...
    html += `
            ...
            <p class="mt-3"><small>Todas as séries foram criadas com sucesso. Comunique-as à AT antes de usar.</small></p>
        </div>
    `;

    dialog.fields_dict.created_series_list.$wrapper.html(html);
    dialog.show();
}
...
function prepare_series_data(frm) {
    ...
    if (!frm.doc.current_sequence) {
        let doc_info = DOCUMENT_TYPES[frm.doc.document_type];
        frm.doc.current_sequence = doc_info ? doc_info.default_sequence : 1;
    }
...
function setup_series_validations(frm) {
    /**
     * Configurar validações da série
     */
...
console.log('Portugal Series Configuration JS loaded - Version 2.0.0 - All Documents Supported');
