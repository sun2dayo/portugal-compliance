...
frappe.ui.form.on('Delivery Note', {
    ...
    refresh: function(frm) {
        ...
        // reabrir uma guia ja submetida, que e precisamente quando este
        // estado mais interessa consultar. Colocado logo no inicio do
        // refresh (confirmado ao vivo em quotation.js que codigo mais
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (r.message) {
                ...
                // WSE), distinto de saber se ESTA guia foi enviada via
                // envioDocumentoTransporte (WDT) - ver
                // show_at_communication_status abaixo.
...
function show_at_communication_status(frm) {
    /**
     * Estado real da comunicação DESTA guia à AT via webservice
     * (envioDocumentoTransporte, permissão WDT) - distinto do indicador
     * de série acima (webservice de séries, WSE). Mesmo padrão de
     * sales_invoice.js.
     */
    ...
    frappe.call({
        ...
        callback: function(r) {
            ...
            const label = {
                Success: __('Guia Comunicada à AT'),
                Failed: __('Falha ao Comunicar Guia à AT'),
            ...
            frm.dashboard.add_indicator(label, color).attr(
                'title',
                __('Estado da comunicação DESTA guia à AT via webservice (envioDocumentoTransporte) - distinto do registo da série mostrado acima.')
            );
        }
    });
}
...
frappe.ui.form.on('Delivery Note', {
    onload: function(frm) {
        ...
        // Code herdado do documento original numa devolução nova.
